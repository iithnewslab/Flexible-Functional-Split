...
qw  = 1;
global data_rate_actual inter_packet_time no_UE packet_interval dynamic_flow packet total_throughput;
%%%------------------------------%%%
no_UE = 100;
...
P = packet;
for df = 1:8
    data_rate_actual(df) = packet_size(df)*sum(P(1,:))*8*1e3; %Kbps 
end
total_throughput = sum(data_rate_actual,"all")/1e6;
end

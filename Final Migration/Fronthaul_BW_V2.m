...
data_Rate =  (700 + 2100*sinc(3*t + 1.7))/2;
plot(t,data_Rate)
IP_DL_TTI = ceil(data_Rate*1024/(1500*8));
...
%     if (PDCP_RLC < Max_BW,RLC_MAC,Split_MAC,MAC_PHY )
    temp_1 = PDCP_RLC(i);
    temp_4 = MAC_PHY(i);
    Dynamic_load(i) = max([temp_1,temp_4]);
...
% plot(t,Split_4)
legend('User Traffic','PDCP-RLC','MAC-PHY','Max-BW','Adaptive-split')%, 'Intra-PHY')
xlabel('t')

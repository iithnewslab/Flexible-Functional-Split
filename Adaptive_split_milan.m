y = csvread('traffic_processed.csv');
user_traffic = 5*smooth(y(150:end));


z = length(user_traffic);
t = linspace(1,z,z);
%% BW requirements
...
IP_DL_TTI = ceil(user_traffic*1e6/(N_TBS_DL*(1500)*8*1000));
...
%% Plots
% line_width = 1;
plot(t,user_traffic,'LineWidth',1)
hold on
plot(t,PDCP_RLC,'--','LineWidth',1)
plot(t,RLC_MAC,'--','LineWidth',1)
plot(t,Split_MAC,'--','LineWidth',1)
plot(t,MAC_PHY,'--','LineWidth',1)
% plot(t,Split_1,'LineWidth',2)
plot(t,Max_BW,'--','LineWidth',1);

plot(t,Dynamic_load,'LineWidth',1)
